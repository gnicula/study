	...
	*/
	...
	public void moveForwardBack(float speed, Vector3f cameraLoc) {
		...
		if ((newLocation.length() < 9.0f) ) {
			if (newLocation.y() >= -0.02f && newLocation.y() <= 0.5f) {
				...
				if (newLocation.y() <= -0.02f) {
					...
					this.setLocalLocation(new Vector3f(newLocation.x(), 0.5f, newLocation.z()));
				}
			}
		}
	}
	...
	public void yaw(float speed) {
		...
		Matrix4f rotationAngle;
		rotationAngle = new Matrix4f().rotation(
				speed, new Vector3f(
						previousUpAngle.x(),
						previousUpAngle.y(),
						previousUpAngle.z()));
