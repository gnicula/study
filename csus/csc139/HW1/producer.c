...
{
        pid_t pid;
        int bufSize; // Bounded buffer size, [2, 512]
        int itemCnt; // Number of items to be produced, [10, 2500]
	...
	// Write code to check the validity of the command-line arguments
        // Check buffer size to be in range 2 to 512
        if(bufSize < 2 || bufSize > 512) {
                printf("Buffer size must be between 2 and 512 (inclusive).\n");
                exit(1);
        }
        // Check item count to be in range 10 to 2500
        if(itemCnt < 10 || itemCnt > 2500) {
                printf("Item count must be between 10 and 2500 (inclusive).\n");
                exit(1);
        }
...
void InitShm(int bufSize, int itemCnt)
{
    int in = 0;
    int out = 0;
    const char *name = "OS_HW1_GabrieleNicula"; // Name of shared memory object to be passed to shm_open
    ...
    int my_sharedm_fd = shm_open(name, O_CREAT | O_RDWR, 0666);
    if(my_sharedm_fd == -1) {
        printf("Producer: shm_open error when opening shared memory object.\n");
        exit(1);
    }

    if(ftruncate(my_sharedm_fd, SHM_SIZE) == -1) {
        printf("Producer: ftruncate error when truncating shared memory object to SHM_SIZE.\n");
        exit(1);
    }
 
    // map the shared memory block for both reading and writing!
    gShmPtr =  mmap(0, SHM_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, my_sharedm_fd, 0);    
    if(gShmPtr == MAP_FAILED) {
    	printf("Producer: mmap failed.\n");
        exit(1); 
    }
